...
import io.lyracommunity.bolt.util.ReceiveBuffer.OfferResult;
...
public class TestReceiveBuffer {
    ...
    @Test
    public void testOrderedPacketLookahead() {
        ReceiveBuffer b = new ReceiveBuffer(4);

        assertEquals(OfferResult.OK_ACCEPTED, b.offer(orderedDataPacket(1)));
        assertEquals(OfferResult.OK_ACCEPTED, b.offer(orderedDataPacket(3)));
        assertEquals(OfferResult.ERROR_LOOKAHEAD, b.offer(orderedDataPacket(4)));
    }
    ...
    public void testBufferOverflow() {
        ...
        for (int i = 0; i < 3; i++) {
            assertEquals(OfferResult.OK_ACCEPTED, b.offer(orderedDataPacket(i + 1, i + 1, "test".getBytes())));
        }
        for (int i = 0; i < 3; i++) {
    ...
    public void testOrderSequenceOverflow2() {
        ...
        for (int i = 0; i < 5; i++) {
            orderSeqNum = SeqNum.increment16(orderSeqNum);
            assertEquals(OfferResult.OK_ACCEPTED, b.offer(orderedDataPacket(i + 1, orderSeqNum, ("test" + i).getBytes())));
        }
    ...
    private DataPacket orderedDataPacket(int orderAndPacketSeqNo) {
        return orderedDataPacket(orderAndPacketSeqNo, orderAndPacketSeqNo, ("test" + orderAndPacketSeqNo).getBytes());
    }
